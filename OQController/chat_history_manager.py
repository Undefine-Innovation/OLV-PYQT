...
def _get_base_chat_history_dir() -> str:
    """Get the absolute path to the chat_history directory"""
    # Get the directory where this script is located
    current_dir = os.path.dirname(os.path.abspath(__file__))
    # Go up one level to the project root, then into chat_history
    base_dir = os.path.join(os.path.dirname(current_dir), "chat_history")
    return base_dir
...
def _ensure_conf_dir(conf_uid: str) -> str:
    ...
    base_dir = os.path.join(_get_base_chat_history_dir(), safe_conf_uid)
...
def get_history(conf_uid: str, history_uid: str) -> List[HistoryMessage]:
    ...
    filepath = _get_safe_history_path(conf_uid, history_uid)
    logger.debug(f"Attempting to load history from: {filepath}")
    ...
    try:
        with open(filepath, "r", encoding="utf-8") as f:
            ...
            logger.debug(f"Successfully loaded {len(messages)} messages from history")
            return messages
    except json.JSONDecodeError as e:
        logger.error(f"JSON decode error in history file {filepath}: {e}")
        return []
    except UnicodeDecodeError as e:
        logger.error(f"Unicode decode error in history file {filepath}: {e}")
        return []
    except Exception as e:
        logger.error(f"Unexpected error loading history file {filepath}: {e}")
        return []
...
def get_history_list(conf_uid: str) -> List[dict]:
    ...
    if not conf_uid:
        logger.warning("No conf_uid provided for get_history_list")
        return []

    try:
        conf_dir = _ensure_conf_dir(conf_uid)
        logger.debug(f"Looking for histories in directory: {conf_dir}")
    except Exception as e:
        logger.error(f"Failed to ensure conf directory: {e}")
        return []
    ...
    try:
        if not os.path.exists(conf_dir):
            logger.warning(f"Configuration directory does not exist: {conf_dir}")
            return []
            
        files = os.listdir(conf_dir)
        logger.debug(f"Found {len(files)} files in {conf_dir}")
        ...
        for filename in files:
            ...
            try:
                ...
                logger.error(f"JSON decode error reading history file {filename}: {e}")
                continue
            except Exception as e:
        ...
        if len(empty_history_uids) > 0 and len(files) > 1:
        ...
        logger.debug(f"Successfully loaded {len(histories)} histories")
        return histories
